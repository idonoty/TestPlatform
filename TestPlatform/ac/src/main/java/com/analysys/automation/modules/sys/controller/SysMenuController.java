...
import com.analysys.automation.modules.sys.service.ShiroService;
import com.analysys.automation.modules.sys.service.SysMenuService;
...
public class SysMenuController extends AbstractController{
    ...
    public Ret nav(){
        ...
        return Ret.ok().put("menuList", menuList).put("permissions", permissions);
    }
    ...
    public Ret save(@RequestBody SysMenuEntity menu){
        //数据校验
        verifyForm(menu);
    ...
    public Ret update(@RequestBody SysMenuEntity menu){
        //数据校验
        verifyForm(menu);
    ...
    public Ret delete(@PathVariable("menuId") long menuId){
        if(menuId <= 31){
    ...
    private void verifyForm(SysMenuEntity menu){
        if(StringUtils.isBlank(menu.getName())){
            throw new AutomationException("菜单名称不能为空");
        }

        if(menu.getParentId() == null){
            throw new AutomationException("上级菜单不能为空");
        }

        //菜单
        if(menu.getType() == Constant.MenuType.MENU.getValue()){
            if(StringUtils.isBlank(menu.getUrl())){
                throw new AutomationException("菜单URL不能为空");
            }
        }
        ...
        if(menu.getType() == Constant.MenuType.CATALOG.getValue() ||
            ...
            if(parentType != Constant.MenuType.CATALOG.getValue()){
                throw new AutomationException("上级菜单只能为目录类型");
            }
            return ;
        }
        ...
        if(menu.getType() == Constant.MenuType.BUTTON.getValue()){
            if(parentType != Constant.MenuType.MENU.getValue()){
                throw new AutomationException("上级菜单只能为菜单类型");
            }
            return ;
        }
    }
